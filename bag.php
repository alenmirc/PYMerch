...
<body>
   <nav class="navbar navbar-expand-lg navbar-light sticky-top" style="background-color: #202020;">
      ...
      <h2 class="mt-5 ml-15 mb-1 text-center" style="color: white; font-family: Titillium Web;">- BAG PRODUCTS -</h2>
     <br>
...
$sql = "SELECT id, product_name, category, users.name, target_file,  product_link FROM product  INNER JOIN users ON product.user=users.user_id WHERE category='2' ORDER BY id DESC;";
$result = $db->query($sql);
print_r($result);
...
<div class="col d-flex justify-content-center">
  <div class="card text-white mb-3" style="width: 18rem; height:30rem; background-color: #202020;">
  <img class="card-img-top border border-dark z-depth-2" src="<?php echo $row['target_file'] ?>" alt="View">
    <div class="card-body">
      <p class="card-text text-center  mb-1">  <?php echo $row['product_name'] ?></p>
      <p style="color:white; font-style: italic; text-align:center; font-size:10px; margin:0px; ">by <?php echo $row['name'] ?></p>
      <div class="d-flex justify-content-center mt-1">  <a href="<?php echo $row['product_link'] ?>" class="btn btn-dark btn-primary ">View</a></div>
    </div>
  </div>
</div>
